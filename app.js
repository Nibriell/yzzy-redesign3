// YZZY — EASY · logica partajată (catalog, coș, efecte)
...
// ---------- coș ----------
let cart = [];
try { cart = JSON.parse(localStorage.getItem('yzzyEasyCart')) || []; } catch (e) {}
const saveCart = () => localStorage.setItem('yzzyEasyCart', JSON.stringify(cart));
...
function addToCart(id, qty = 1) {
  ...
  const p = byId(id);
  toast(`<b>●</b> ${p ? p.name : 'Produs'} — adăugat în coș`);
}
...
// ---------- logo (wordmark + punct albastru) ----------
function logoMark() { return 'yzzy<span class="d"></span>'; }
...
function renderNav(active) {
  ...
  document.getElementById('nav').innerHTML = `
    ...
    <div class="nav-cta">
      ...
      <a class="btn btn-blue" href="catalog.html">Cumpără ușor</a>
    </div>`;
...
function renderMarquee(el) {
  const items = ['Garanție 2 ani', 'Retur gratuit 30 de zile', 'Livrare în 24h', 'Plata în rate', 'Verificat de profesioniști'];
  const half = `<span>${items.join('</span><i></i><span>')}</span><i></i>`;
  el.innerHTML = `<div class="marq-track"><span style="gap:40px">${half}${half}</span></div>`;
}
...
function renderFooter(el) {
  ...
  <div class="foot-grid">
    <div class="foot-brand">
      ...
      <p>Telefoane și gadgeturi recondiționate, verificate și garantate 2 ani. Simplu, sigur, fără bătăi de cap. YZZY MOBILE SRL · Suceava, Str. Ana Ipătescu, Nr. 5.</p>
    </div>
  ...
  <div class="foot-mega">yzzy<span class="d">.</span></div>
...
function initCursor() {
  ...
  const loop = () => { x += (tx - x) * 0.25; y += (ty - y) * 0.25; c.style.left = x + 'px'; c.style.top = y + 'px'; requestAnimationFrame(loop); };
  loop();
...
function initSearch() {
  ...
  if (!ov) {
    ...
    ov.innerHTML = `<button class="icon-btn search-close" aria-label="Închide">×</button>
      <input type="text" placeholder="Caută un telefon…" aria-label="Caută produse">
    ...
    input.addEventListener('input', () => {
      ...
      res.innerHTML = hits.length ? hits.map(p => `<a href="produs.html?id=${p.id}"><span>${p.brand} ${p.name}</span><b>${fmtLei(p.price)}</b></a>`).join('') : '<a style="color:var(--ink-soft)">Niciun rezultat.</a>';
    });
...
function sealHTML() {
  return `<div class="seal" aria-hidden="true">
    ...
    <text style="font-family:'Space Mono';font-weight:700;font-size:9px;letter-spacing:2.2px;fill:var(--blue)">
    <textPath href="#circ">GARANȚIE 2 ANI · VERIFICAT DE PROFESIONIȘTI · YZZY ·</textPath></text></svg>
